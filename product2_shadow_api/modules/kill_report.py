...
class KillReportGenerator:
    ...
    def generate(self, audit_results: list[dict], shadow_apis: list[dict],
        ...
        if not output_filename:
            ...
            output_filename = f"orkzoid_api_report_{timestamp}"
    ...
    def _generate_nginx_rules(self, dangerous: list[dict], shadow_apis: list[dict]) -> str:
        ...
        lines = ["# Orkzoid — Auto-generated nginx deny rules", f"# Generated: {datetime.now(timezone.utc).isoformat()}", ""]
        paths_blocked = set()
    ...
    def _generate_iptables_rules(self, target: str) -> str:
        ...
        return "\n".join([
            "# Orkzoid — Auto-generated iptables rules",
            f"# Target: {target}",
            ...
            "iptables -A INPUT -p tcp --dport 80 -j LOG --log-prefix 'ORKZOID_HTTP: '",
            "iptables -A INPUT -p tcp --dport 443 -j LOG --log-prefix 'ORKZOID_HTTPS: '",
        ])
    ...
    def _build_markdown(self, target, audit_results, shadow_apis, dangerous, gdpr_risks, nginx_rules, iptables_rules) -> str:
        ...
        lines = [
            "# 🛡️ Orkzoid Shadow API Attack Surface Report", "",
            f"**Target:** `{target}`  ",
            f"**Generated:** {now}  ",
        ...
        lines.extend([
            ...
            "---", "",
            "*Report generated by Orkzoid Shadow API Attack Surface Management*", "",
        ])
    ...
    def _build_json(self, target, audit_results, shadow_apis, dangerous, gdpr_risks, nginx_rules, iptables_rules) -> dict:
        ...
        return {
            "report": {
                "tool": "Orkzoid Shadow API Scanner",
                "version": "1.0.0",
